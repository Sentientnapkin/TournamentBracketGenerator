...
public class SingleElimination implements ActionListener {
    ...
    public SingleElimination(ArrayList<String> players, int boX) {
        ...
        createPanels(bracketSize);

        populateInitalRounds(players, boX, bracketSize);
    ...
    private void populateInitalRounds(ArrayList<String> players, int boX, int bracketSize) {
    ...
    private void createPanels(int sizeTemp) {
    ...
    public void actionPerformed(ActionEvent ae) {
        ...
        for (int panelIndex = 0; panelIndex < panels.size() - 2; panelIndex++) {
            for (int matchIndex = 0;
                    matchIndex < panels.get(panelIndex).getComponentCount();
                    matchIndex++) {
                System.out.println(panelIndex);
                if (!(panels.get(panelIndex).getComponent(matchIndex) instanceof Match)) continue;
                Match match = (Match) panels.get(panelIndex).getComponent(matchIndex);
                if (match.isNotFinished()) continue;
                String won = match.getWinner();
                if (panelIndex + 1 == panels.size() - 2) winner.setText(won + " wins!");
