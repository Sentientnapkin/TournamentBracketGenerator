...
public class RoundRobin implements ActionListener {
    ...
    public RoundRobin(ArrayList<String> players, int boX) {
        ...
        for (int playerIndex = 0; playerIndex < players.size(); playerIndex++) {
            ...
            for (int playerIndex2 = playerIndex + 1;
                    playerIndex2 < players.size();
                    playerIndex2++) {
    ...
    public void actionPerformed(java.awt.event.ActionEvent e) {
        if (!e.getActionCommand().equals("Refresh")) return;
        for (int roundIndex = 0; roundIndex < matchesPanel.getComponentCount(); roundIndex++) {
            ...
            for (int matchIndex = 0; matchIndex < panel.getComponentCount(); matchIndex++) {
                ...
                TournamentScorePanel winnerScore =
                        (TournamentScorePanel) totalScores.getComponent(players.indexOf(winner));
                winnerScore.addWin();
                TournamentScorePanel loserScore =
                        (TournamentScorePanel) totalScores.getComponent(players.indexOf(loser));
                loserScore.addLoss();
            }
        }
    }
